...
class fullyConectedLayer:
	...
	def __init__(self, input_len , layer_len ):
		...
		self.excitatory = numpy.random.uniform(3.8,6.0,size=(self.input_len, self.layer_len))
	...
	def backward(self, reward): #TODO update the inhibitory
		...
		if reward > 0.0:
			...
			self.excitatory =  self.excitatory + (self.max_weight - self.excitatory * self.spiked) * reward
		...
		if reward < 0.0:
			#If the reward it's negative, we want to make the weights responsable of firing smaller.We do this proportinal to 
			#the difference to the mimun weight and to the reward
			self.excitatory =  self.excitatory + (self.min_weight + self.excitatory * self.spiked) * reward
		return
